...
#%% APPLY OVER SAMPLING USING RANDOM RESAMPLING METHOD
print('OVERSAMPLING')
...
#get predictions
y_pred_gbm_opt = tuning.predict(X_test)
gbm_opt_report = classification_report(y_test, y_pred_gbm_opt)

print(f"SMOTE Gradient Boosting Classification Hyperameter Optimized Report : \n{gbm_opt_report}")
#Gradient Boosting model is best model based on F-1 score
best_model=tuning;

#PREDICT PROBABILITIES
y_train_opt_preds, y_test_opt_preds=predict_p(tuning, X_train, X_test);

# GET AUC, ROC scores and curve
auc_opt_train, auc_opt_test=auc_roc(y_train, y_train_opt_preds, y_test, y_test_opt_preds, 'SMOTE_GBM_OPT');

#FEATURE IMPORTANCES FOR GRADIENT BOOSTING
#----------------------------------------------------------------------------
feature_importances_gbm_opt=get_fi(tuning, X_orig, 'SMOTE_gbm_OPT_feature_importances');
